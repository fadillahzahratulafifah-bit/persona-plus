...
export default function CostumesManagementPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      alert("Gagal menambahkan kostum.");
    }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {isAdding && (
        <div className="bg-card border rounded-2xl p-6 shadow-sm animate-in slide-in-from-top-4 duration-300">
          <h3 className="font-bold text-lg mb-4">Kostum Baru</h3>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {costumes.map(c => (
            <div key={c.id} className="bg-card border rounded-2xl p-5 flex flex-col gap-3">
              ...
              <Button variant="outline" size="sm" className="mt-auto border-destructive text-destructive hover:bg-destructive hover:text-white rounded-full" onClick={() => handleDelete(c.id)}>
