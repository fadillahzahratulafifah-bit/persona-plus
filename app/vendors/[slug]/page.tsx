...
export default async function VendorDetailPage({ params }: { params: { slug: string } }) {
  const { data: vendor, success, error } = await VendorService.getVendorBySlug(params.slug);
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start md:items-center mb-6">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <h1 className="text-2xl md:text-3xl font-bold font-heading">{vendor.name}</h1>
                  <CheckCircle className="w-5 h-5 text-success fill-success/20" />
                </div>
                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <Star className="w-4 h-4 text-warning fill-warning" /> {vendor.rating} ({vendor.reviewsCount} Reviews)
                  </span>
                  <span className="flex items-center gap-1">
            ...
            <div className="bg-card p-6 rounded-3xl border shadow-sm sticky top-24">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-success font-medium">Tersedia</span>
                </div>
              </div>
